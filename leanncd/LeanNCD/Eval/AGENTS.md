...
| Add a new dtype/semiring | `Contract.lean` — new `Combine.*` + `combineFor` arm. For a new CARRIER (not a semiring over an existing one) that is only the legacy-evaluator half: follow the f32 slice's shape instead — `DSL/Ast.lean`'s `TensorElementType`, `Plan/Check.lean`'s `checkAssignCore` `kind` parameter and algebra table, `Plan/Dense.lean`'s `ScalarKernelOps` instance, a graph worker beside `Dense32.lean`, a named boundary beside `Adapter32.lean`, and a storage-kind guard at **every** door in the §3.5 table of `papers/f32_evalplan.md` |
