...
| `Error.lean` | closed diagnostics: `PlanError` (checker), `PositionalInputError` (runtime), `CapabilityError` (capability rejection), `InputSignatureError`/`InputBindingError`/`BindingsError` (boundary), `PlanRunCause`/`PlanRunFailure` (run-time). `PlanCompileCause`/`PlanCompileFailure` are NOT here any more — F3 moved them to `EvalPlan.lean` |
