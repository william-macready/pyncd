...
## Pitfalls
- **The checked backend executes Boolean/tropical/unary semantics; the experimental JAX backend
  REJECTS them** — `Plan/Executable.lean`'s `checkJaxAssignSupport` fails loud (located, typed) on a
  Boolean destination, a Boolean source, `admittedAlgebraMax`/`Min`, or a `ReadPlan.unary` read
  before any Python, candidate, or `ExecutionEvidence` exists. Do not read "Boolean outputs are
  admitted" as "JAX runs Boolean plans" — there is no JAX Boolean execution.
