...
| `Dense.lean` | Dense interpreter for one checked operation, over positional `DenseTensor` storage — `runDenseAssignAt` (context-indexed primitive, built from named folds `factorFold`/`reductionFold`/`termFold` matching architecture doc §2.2) and `runDenseAssign` (its empty-context wrapper). `gatherFactor` applies a `ReadPlan.unary` function after the OOB zero-pad via the shared `UnaryOp.applyChecked` (`Eval/Error.lean`), failing loud on a domain violation as `PositionalInputError.unaryDomain` — so `runDenseAssignAt` is `Except`-threaded. `runDenseScatter` (S-A Task 4) is the scatter worker: it runs the compute half's source domain, then places each computed value into the `fill`-initialized destination through the `outCoeffs`/`outBias` placement map, resolving coincident writes under the `reduce` collision policy |
...
- **The write-geometry surface MUST stay exhaustive over `WriteRowKind`** (`Plan/Scan.lean` + `Plan/Compile.lean`'s base-write placement loop + `test/Eval/Plan/ScanTest.lean`'s frozen `stepWriteRowsOkNoClause1` oracle) — every consumer that *matches* a row leaves a fourth constructor uncovered, so adding one is a compile error there. **Nine sites, seven of them production:** `baseWriteRowsOk`'s positional cover, both of `stepWriteRowsOk`'s non-advancing clauses, `freeExtentsAgree`, `pinnedLiteralsInRange` and `compileScan`'s pinned-literal loop each spell out all three constructors plus `none` over a single scrutinee; `writesCollide` matches a PAIR and is exhaustive in its first component only (three of its arms use `_` for the second), which is enough — both `some (.strided _), _` and `some (.pinned _), some (.strided _)` would be uncovered; and the `stepWriteRowsOkNoClause1` oracle repeats two of those matches in the test suite, which is where the last two errors come from. They do NOT all arrive in one build: `Plan/Scan.lean`'s six are reported together, and `Plan/Compile.lean`'s one and `ScanTest.lean`'s two surface only once those six are discharged, since a module that fails to compile blocks its dependents. **Do not restore a `| _ =>` arm at any of them.** The exhaustiveness obligation IS the tripwire: it turns adding a row kind into a compile error at each site that would otherwise silently exempt it from that site's value check, and **no value-comparing test can catch a wildcard being put back** — a new kind falling through `| _ =>` produces no wrong value, it produces an unchecked one. **The residual limit is worth stating plainly, because it bounds what the tripwire buys: it forces a DECISION at each value check, it does not constrain the decision.** An author who discharges the nine errors by mechanically writing `| some (.strided ..) => true` at each one, mirroring the neighbouring `.free`/`.advancing` arms, satisfies every compiler obligation and reproduces the historical defect exactly — the new kind exempted from every value check, just deliberately this time. Each arm needs its own answer, derived from `papers/pre_scatter_backend_audit.md`'s cell table rather than from its neighbours; `papers/scatter_affine_lhs_writes.md` §3.4 carries the corresponding warning for `classifyWriteRow`, which the tripwire cannot reach at all — it requires a WITNESS FUNCTION matching exhaustively over `WriteRowKind`, with both a `contextWidth = 0` and a `contextWidth > 0` witness per constructor (a `#guard` set naming the existing constructors never mentions the new one, and a lone base-phase witness cannot separate the correct `outputPos = p - contextWidth` from a broken `outputPos = p`). Two sites can never be tripwired: `classifyWriteRow` *produces* the kinds (its catch-all scrutinises the nonzero-coefficient list, not a `WriteRowKind`) and `causalAdvancingRow` classifies read rows and never mentions the type — a new kind must be admitted in `classifyWriteRow` deliberately, since nothing will remind you. The boundary rule is decided by EQUALITY rather than by matching (`baseWriteRowsOk`'s advancing-pin clause, and the `baseWriteNotAtBoundary` guard in `compileScan` that still restates it inline — the one surviving duplicate of a `Scan.lean` rule): a comparison against `some (.pinned 0)` carries no exhaustiveness obligation, so a new kind is simply unequal and the write is rejected, safe by default but with no compiler prompt at either site. **Owed fix, owned by the deferred scan-scatter slice** (which reopens `baseWriteRowsOk` to admit a strided row anyway; note the scatter work is TWO slices — see `papers/scatter_affine_lhs_writes.md` §1): extract that advancing-pin clause into its own named `Scan.lean` predicate and have both `baseWriteRowsOk`'s third conjunct and `compileScan`'s guard call it. Calling the WHOLE of `baseWriteRowsOk` from `compileScan` is not the fix and was rejected deliberately — it compiles and the full suite passes, because clauses 1-2 already hold there, but only via a non-local invariant spanning `compileScan`'s phases (`inconsistentStateRank` fixes one slot count per state, `stateShapes` is built at that count, and the base loop emits one coefficient row per slot). Asserting them at that point would re-report a hypothetical compiler bug — which `checkScanPlan`'s `writeGeometryNotAdmitted`/`writeCoeffRankMismatch` exist to catch on the `invalidPlan` channel — as a source-facing `baseWriteNotAtBoundary` naming a boundary fault that is not the real fault. Same drift shape as the `scatterOutShape` row above; the pinned-literal half is now a real call, mutation-verified — deleting `pinnedLiteralsInRange`'s pinned-row rule breaks `Eval.Plan.ScanCompileTest`'s two `baseWritePinOutOfRange` guards, which the former inlined copy survived unchanged.
...
| Understand why a source scatter was rejected | An affine or diagonal LHS is reclassified into a `Stmt.scatter` by `Compile.lean`'s `lowerArith`, then gated by that file's scatter checks. Match the `CapabilityError` constructor: `scatterOrAffineLhs` = a constant-affine slot (`Out[3]`) or any affine slot on a path with no scatter lowering; `multiAxisScatterLhs` = a row naming more than one source axis (`Out[i+j]`); `scatterOptsNotAdmitted` = a collision policy other than the admitted reject-on-collision (S-A Task 5); `predicateScatterDest` = a `predicate`/`bool`-declared scatter destination (deferred: the reference `evalScatter` picks its algebra from `rhs.agg` alone and never sees `decls`, so a Boolean destination would silently diverge; rejected at the same tier that refuses tropical fills, and for the same reason). A scan-block scatter (strided write into scan state, S-B) is still rejected outright and stays so. Fixtures: `test/Eval/Plan/CompileTest.lean` (the capability-boundary rejections), `test/Eval/Plan/ScanCompileTest.lean` (the scan-block rejections S-A does not lift) |
| Check that a compiled scatter still MATCHES the legacy evaluator | `test/Eval/Plan/DifferentialTest.lean`'s `scatterPrograms` corpus — 8 curated accepted scatter programs (strided/diagonal/2D/RHS-only-axis placement shapes plus scatter-output-consumed-downstream cases), each run through the checked-vs-reference differential (`planAgreesForDecls`, exact `envEq`) plus an observed-value assertion, kept SEPARATE from `enumPrograms` for the reason stated twice there. Source-reachability and per-field plan structure of the same shapes are pinned by `test/Eval/Plan/ScatterCompileTest.lean` (S1–S9); the checker and dense worker are pinned by `ScatterCheckTest.lean`/`ScatterDenseTest.lean` |
