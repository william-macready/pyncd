...
| Check that a compiled scan still MATCHES the legacy evaluator | `test/Eval/Plan/DifferentialTest.lean`'s scan section — the eight-point execution matrix (`scanParityCheck`) over every `ScanCompileTest.lean` acceptance fixture plus the curated `enumScanCases` corpus gate (17 cases: 13 accepted, 0 `unsupportedNonlin`, 4 `unsupportedAgg` — was 9 / 4 / 4 at Wave F F4 authoring time; thread 4 Task 4 admitted pointwise/axiswise scan-block nonlinearities and moved four cases from the rejection into the accepted column). Parity holds only for the fragment where the checked Jacobi semantics and the legacy worker AGREE (proposal §10 Law 1) — see that section's scope note. Named-boundary FAILURES (missing/mis-shaped input at a scan capture, warning preservation) are in `test/Eval/Plan/AdapterTest.lean` Checks 11–16 |
