...
| `Compile.lean` | the source compiler — capability preflight, per-statement lowering, scan specialization, nonlinearity-axis resolution, and `prepareEvalPlan`. The latter gets its direct-schedule authority exclusively from `Pipeline/ScheduledValidation.lean`'s `validateScheduled`: duplicate-safe declarations, source/destination ranks, predicate outputs, producer order, declaration-derived sizes, and external-name order all precede capability preflight, signature validation, and shape inference; cached `sched.env`, `sched.extNames`, and `sched.explicitSizes` are never trusted |
| `Adapter.lean` | named ↔ positional runtime boundary — `pack`/`unpack`/`runPreparedDense`. `unpack` takes the whole `PreparedPlan` and is `Except PlanRunCause`-valued (whole-branch review round 4): it requires the positional result store to have EXACTLY `plan.raw.tensorSigs.size` entries (`PlanRunCause.resultStore (.storeArityMismatch …)`) BEFORE resolving any binding — slot validity is a fact about the checked plan, not about the store a caller hands over, so an oversized store no longer licenses a binding the plan rejects — and then resolves `materializedNames` through the shared `PlanBindings.materializedWith`, so a binding naming a slot outside the plan fails loud before any name is published (`PlanRunCause.materialization`) instead of publishing a fabricated empty tensor under a real output name. `runPreparedDense` propagates that cause verbatim, so a direct `unpack` call and the full pipeline report the identical value |
...
all source/destination rank checks, predicate-output checks, producer order, declaration-derived
sizes, and ordered external names; neither boundary trusts the cached schedule fields.
...
| `evalScheduled` (`Eval.lean`) | `Entry.lean`, scheduled-program callers | compiler-independent worker; returns the same success/failure report pair as the source entry. Whole-branch review: it re-runs the shared `buildDeclEnv` over `sched.decls` FIRST, so a hand-built schedule declaring one tensor-bearing name twice is `EvalError.compile (.duplicateTensorDecl …)` rather than being evaluated under whichever kind `combineFor`'s first-match scan happens to pick — the same `CompileError` `prepareEvalPlan` reports as `sourceInvariant`. An `.axis` sharing a predicate's name is a different namespace and stays legal. Its size-inference seed is likewise re-derived (`declaredAxisSizes sched.decls`, not `sched.explicitSizes`) — the same rule `prepareEvalPlan` applies, so a stale cached map cannot make the two backends disagree. It also enforces the shared READ-ARITY and predicate-output invariants over every scheduled assignment (`checkScheduledReadRanks`/`checkPredicateOutputs` — the same rules AND the same `.plain`/scan-`base ++ recur` traversal Step 0 uses), so a read contradicting its declaration is `EvalError.compile (.rankMismatch …)` instead of being resolved against whatever rank the supplied input happens to have, and a `predicate` destination carrying a nonlinearity or a non-`sum` aggregation is `EvalError.compile (.predicateNonlin/.predicateAgg …)` instead of being executed with `applyNonlin` mapped over Boolean data and the `agg` silently ignored. Order is the source pipeline's own: `buildDeclEnv`, then read ranks, then the predicate rule (each in statement source order), then size inference and execution |
