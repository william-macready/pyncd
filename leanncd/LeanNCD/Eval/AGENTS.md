...
- **An unsized scan iteration axis is an error naming its own fix** — `Scan.evalScan` reports "unsized iteration axis 'l' (uid N) — pin it with ``axis l : ℕ = N``, or ensure some read fixes its extent". Two adjacent cases are deliberately NOT covered: an axis pinned explicitly to `0` still yields `L = 0` (a stated intent, not a sizing gap), and `axis l : ℕ[3]` pins **nothing at all** — the kind-carried size is write-only, so it behaves identically to declaring no axis (see `../DSL/AGENTS.md`, audit finding H).
