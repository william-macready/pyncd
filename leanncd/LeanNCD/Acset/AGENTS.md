...
- **`encodeSize` is partial over `SizeExpr`** — errors on compound expressions (`.add`/`.sub`/`.mul`/`.div`); only `.lit`/`.var` round-trip through CSV. An `SBrInstance` with compound symbolic sizes fails to serialize (`Except.error`, not a crash) unless the caller handles it.
