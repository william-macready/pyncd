...
- **`splitStmt`'s linear (`.identity`) half must NOT carry the original `.freeNorm` marker** (found and fixed 2026-08-21, leanncd Thread 4 Task 3). `splitStmt` (`Lowering.lean`, Phase 6) splits any non-`.identity` statement into a linear step (writes a fresh `%nl{uid}` intermediate, `nonlin := .identity`) plus a nonlin step (reads that intermediate, carries the original `Nonlin`) — and BOTH steps used to reuse the exact same `slots` list, so a `.freeNorm`-marked axiswise statement (e.g. `Y[q, s.] := softmax(A[q, s])`) left the marker on the linear half too, even though `.identity` never needs it (`resolveNonlin`'s `.identity` branch never inspects `slots`, and the contraction evaluator treats `.free`/`.freeNorm` identically). This was dormant/unobservable before Thread 4: `Eval/Plan/Compile.lean`'s `checkLHSSlot` unconditionally rejected every `.freeNorm` slot, so no marked program could reach the Plan compiler at all. Thread 4 Task 3 relaxes that rejection (to make `.axiswise` reachable) — which surfaced this: the Plan compiler's own `resolveNonlinAxis` treats a `.freeNorm` marker on an `.identity` statement as user error, and without a fix it fired on `splitStmt`'s own linear-step artifact for every real axiswise program, making the whole feature unreachable from surface syntax. Fixed by degrading `.freeNorm → .free` on the linear step's OWN slots only (`linSlots`, `Lowering.lean` in `splitStmt`) — the nonlin step keeps the original marker unchanged, since that is the statement `resolveNonlinAxis` needs to see it on. See `test/Eval/Plan/NonlinCompileTest.lean`'s end-to-end `normalize`/`softmax`/`l2normalize` fixtures, which fail without this fix.
