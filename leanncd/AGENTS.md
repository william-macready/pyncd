...
| What the checked-plan backend does NOT support, and the write-geometry/boundary audit of it | `../papers/pre_scatter_backend_audit.md` — findings-only audit taken before the Scatter + affine LHS feature; read it before touching `Eval/Plan/Scan.lean`'s geometry predicates or the `Adapter`/`Prepared`/`Executable` binding surface. Its `[snippet]` evidence is the twelve tracked `spikes/Axis*Probe.lean` files. Companion inventory: `../papers/backend_missing_functionality.md` |
| What to DO about the audit — ordering, which slice owns what, GPT carve-outs, simplification/dead-code list | `../papers/post_audit_roadmap.md`. Slice 1 (write-geometry hardening) is specified and implementable; the Scatter slice is deliberately scoped-only, because its task list is the compile-error list Slice 1's tripwire produces. **One scope question gates it: are affine LHS writes wanted in `base` blocks or step-only?** |
| Feasibility experiments: checked `EvalPlan` -> JAX | `experiments/jax_bridge/` — four isolated runners: upstream `NetSpec`, the narrow real-`jnp.einsum` smoke, the ordered affine-reference curated suite, and the full 3,832-case PropertyOracle corpus. The corpus runner explicitly builds non-default `JaxExperiment` plus `Tests`, compares every materialized output eagerly, and JIT-checks feature representatives; generated data stays under ignored `.cache/`. These are empirical backend checks, not production imports or scan lowering. **This backend renders ONLY real `f64` sum-product with plain reads, and only for CONTEXT-FREE assignments**: a Boolean destination, a Boolean source, tropical max/min algebra, a unary read, an Iverson factor, and a contextful assignment (non-empty `AssignPlan.contextShape`) are all rejected with located typed errors before any Python or evidence, and `einsumOnly` additionally rejects a zero-padded read whose source extent disagrees with its own iteration extent (`labelExtentMismatch` — the rendered einsum would be a different-shaped function; `affineReference` renders it correctly) (see that directory's README) — admitting Boolean outputs in the checked backend did NOT add JAX Boolean execution, and no JAX kernel takes a runtime context coordinate. |

### Global Invariants (Contracts)

- **Fail loud, never silently drop semantics** — the intended convention across `DSL/` and `Eval/`: cyclic dataflow, non-injective scatters, nonlinear scatters, inconsistent scan axis orders, unsized axes are all explicit errors, not silent miscompiles or wrong answers. ⚠️ **Read this as "enforced where audited, aspirational where not."** This line previously stated the convention as achieved; the 2026-07-30 audit (`papers/semantic_payload_audit.md`) found **five past violations of it** — `evalScan` panicked on an unsized iteration axis instead of erroring, `brOpOfIdx` mapped any unknown tag to `.contract`, `writeSBr` swallowed CSV encode errors, `checkScatterNonlin` accepted `.recurMorphism` then discarded it, and (historically) `splitNonlins` dropped `rhs.agg`. Wave A closed six findings; **finding #6 (the boundary *decoder* defaults — `realizeStMat` zero-fill, `realizeBrBaseP`, `AcsetCodec`, `realizeSBr`) is still UNAUDITED.** The `splitNonlins` `agg`-drop entry is retained as accurate history — `splitNonlins` is now off-chain (regression-only helper in `Pipeline/Lowering.lean`; the production nonlinearity split happens at the `route` boundary via `physicalizeForRoute`), and the drop is provably vacuous even there (a non-sum `agg` statement has `nonlin = .identity` by construction, so the split never carries a droppable non-sum `agg`) — but it was a real bug at the time of the audit, and its lesson (a split that reconstructs a per-node body must carry every field of the source, not just the ones the split's arithmetic uses) still applies. Treat the convention as a goal to verify, not a guarantee to rely on.
...
#### Never
- Treat an *untracked* `spikes/*.lean` as stable or reviewed — the directory is gitignored precisely because it's scratch work. The thirteen **tracked** files are the exception and are deliberate: `BrNF.lean`, plus the twelve pre-Scatter audit probes, which are review-gated evidence for `../papers/pre_scatter_backend_audit.md` and are expected to reproduce byte-identically against their committed `.output.txt`. ⚠️ **Nothing in CI compiles any of them** (they are off the default build), so they can rot silently — if one stops reproducing, suspect that the code moved and re-derive, rather than assuming the spike was wrong.
