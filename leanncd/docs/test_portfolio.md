...
| Scatter statements through the CHECKED backend (parity) | upsample, diagonal | `Eval.Plan.DifferentialTest`'s curated `scatterPrograms` corpus — **8 entries** (S-A Task 6): strided/diagonal/2D/RHS-only-axis placement shapes (SA1–SA4), assign-then-scatter (SA5), and three programs whose scatter output is consumed downstream — reduced to a scalar (SA6), affine-read (SA7), fed into a contraction (SA8). Each runs checked-vs-reference agreement (`planAgreesForDecls`, exact `envEq`) plus its observed value; the length is pinned by `#guard`. Deliberately OUTSIDE `PropertyOracle.enumPrograms`, whose generated size is pinned at 3832. Source-reachability/per-field plan structure of the same shapes: `ScatterCompileTest` (S1–S9) |
