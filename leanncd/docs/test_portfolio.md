...
| SB1–SB7 | **affine scatter writes into scan state**: strided base, strided recurrence, even/odd base interleave, contraction-before-placement, non-trailing advancing dimension, two affine non-advancing dimensions, and two scans with one S-B node | exact histories, shapes, and materialized key sets pinned in `Eval.Plan.DifferentialTest.scanScatterPrograms` | seven source-generated programs compared pairwise across checked plan, legacy scan evaluator, and independent scan-free oracle; exact corpus length 7 | `[N]` `[E]` |
...
| Scatter statements through the CHECKED backend (parity) | upsample, diagonal | `Eval.Plan.DifferentialTest`'s curated `scatterPrograms` corpus — **9 entries** (S-A Task 6), including shifted stride SA9 under the aligned extent rule. `scanScatterPrograms` adds **7** S-B source scans and compares exact key/shape/value results across checked plan, legacy evaluator, and independent scan-free oracle. Both are separate from `PropertyOracle.enumPrograms`, whose generated size remains pinned at 3,832. |
...
| Scans / recurrence | coupled scan, affine state placement | RC2, RC4, RC5 (maxreduce-in-scan), SB1–SB7 |
