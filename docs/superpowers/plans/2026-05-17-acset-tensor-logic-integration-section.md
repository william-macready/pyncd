...
| Generated by `TensorProgram.to_morphism()` | Generated by `TensorProgram.to_instance()` (proposed) |
...
## Subsection 5: Schema Revision — `operator_tag`

**Purpose:** State the one schema change needed to close the remaining gap, and show it is additive and non-breaking.

**Content to write:**

Opening:

> The only schema change needed to accommodate the tensor logic integration is adding an `operator_tag` attribute to $\mathcal{S}_{Br}$:
>
> $$\texttt{Array} \xrightarrow{\texttt{operator\_tag}} \texttt{OpTag}$$
>
> where `OpTag` is defined only for output arrays (`is_input = False`) and is undefined (partial map) for input arrays.

`OpTag` enum values:

| `OpTag` | `TensorEquation.operator` | Pyncd class |
| --- | --- | --- |
| `Identity` | `Identity()` | reindexing — no base computation |
...
| `Embedding` | `Embedding(...)` | discrete lookup |
...
> This is an additive change to $\mathcal{S}_{Br}$ — existing instances without `operator_tag` remain valid as partial instances. No existing tables change structure. The mapping `TensorEquation.operator → OpTag` is injective for all currently-defined operator types and is extended by adding a row to this table when a new operator subclass is introduced. This resolves the schema rigidity concern at minimal cost: the enum is an attribute value, not a schema entity type, so adding a new variant requires no migration of existing instances.
...
- `TensorProgram.to_instance()` is proposed as the generation method but does not yet exist in the codebase. The plan names it as a proposed method; the text should reflect this with "proposed" language.
...
1. The section title is **"Integration with Tensor Logic"**. It comes after the current "Next Steps" section (i.e., at the very end of the file).

2. Add a horizontal rule `---` before the section heading to match the rest of the file.

3. `TensorProgram.to_instance()` should be referred to as "proposed" since it does not yet exist.
