...
| ✅ | **Wave C EvalPlan proposal** — architecture, phase/workstream breakdown, correctness laws, acceptance criteria (design only at the time; implementation followed as C0-C4/C6, see below) | 2026-08-03, revised 2026-08-05 | `papers/wave_c_evalplan_proposal.md` (896ae22, revised 3ebdee1) |
...
| ✅ | **Wave F F0** — executable scan contract: a compiler-checked source-rejection classifier over bare `ScanStmt`s (`Eval.Plan.ScanContractTest`'s `classifyScanStmtF` etc.), plus mechanical legacy-behavior fixtures added to `Eval.ScanTest` covering external read, deep history, extent zero/one, and zero/nonzero base-pin substitution, and two novel findings — a Jacobi/Gauss-Seidel snapshot-safety leak and a base-write collision resolved last-write-wins — each pinned as current legacy behavior against a real `evalScan` run and a passing mutation, both slated to change once Wave F's checked scan worker lands | 2026-08-08 | full cached `lake build` green (8,640 jobs) |
| ✅ | **Wave F F1** — contextual local kernel: generalized `AssignPlan`/`TermPlan` with `contextShape`/`contextPos` fields, partition/projection checking in `checkAssign`, `runDenseAssignAt` (context-indexed primitive), `PositionalInputError.contextShapeMismatch` (runtime validation), and context-sensitive affine-read fixtures (`Eval.Plan.KernelDenseTest`); every existing Wave C plan test remains green, top-level assignment semantics and raw graph ordering unchanged | 2026-08-08 | full cached `lake build` green (8,640 jobs) |
