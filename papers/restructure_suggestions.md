...
| ✅ | **Wave F F0** — executable scan contract: a compiler-checked source-rejection and policy classifier over bare `ScanStmt`s (`Eval.Plan.ScanContractTest`'s `classifyScanStmtF` etc.), plus mechanical legacy-behavior fixtures added to `Eval.ScanTest` covering external read, deep history, extent zero/one, and zero/nonzero base-pin substitution, and two novel findings — a Jacobi/Gauss-Seidel snapshot-safety leak and a base-write collision resolved last-write-wins — each pinned as current legacy behavior against a real `evalScan` run and a passing mutation, both slated to change once Wave F's checked scan worker lands | 2026-08-08 | full cached `lake build` green (8,640 jobs) |
