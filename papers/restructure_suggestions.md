...
| ✅ | **Wave C C3** — checked graph vertical slice: the raw graph IR `RawEvalPlan` (`Plan/Graph.lean`), the graph-wiring checker `checkPlan` and private-constructor `CheckedEvalPlan` (`Plan/Check.lean`), the `PositionalInputError.arityMismatch` diagnostic (`Plan/Error.lean`), and the graph interpreter `runDensePlan` (`Plan/Dense.lean`), with 33 `#guard` fixtures across two test modules (`Eval.Plan.GraphCheckTest` 14, `Eval.Plan.GraphDenseTest` 19), registered in the default build target | 2026-08-06 | full cached `lake build` green (8,632 jobs) |
...
> inference), C2 (checked local kernel vertical slice), and C3 (checked graph vertical slice) are
> done. Wave E's diagnostics/report prerequisites are complete, and the detailed architecture is
> written up in [`wave_c_evalplan_proposal.md`](wave_c_evalplan_proposal.md) (design-only beyond
> C0/C1/C2/C3). C4 (source compiler and representation boundary) is next. Wave C is a
