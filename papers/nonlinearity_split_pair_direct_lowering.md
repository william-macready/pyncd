...
| 6 | `.plain (.scatter …)`, `nonlin ≠ .identity`, **OR** `.plain (.assign …)`, `nonlin ≠ .identity` **AND** `slotsBecomeScatter` | **no** — `checkScatterNonlin` rejects both spellings first, byte-identical payload | **reject; never a silent copy.** At least two known doors closed (implementation review, 2026-08-26): an `.assign` with an `.affine` or diagonal LHS would otherwise take the split arm and silently drop the affine placement (`LHSSlot.toReadIdx` maps `.affine _ => none`). A third door — `.iterAt`/`.iterNext` LHS slots on a `.plain (.assign …)`, which `toReadIdx` also collapses, discarding the pinned literal/shift — is known open, not yet closed; see `LeanNCD/DSL/AGENTS.md`'s case table and the slice's SDD ledger for the reproduction. **A fourth door, MORE SEVERE, found in the Task-2-slice whole-branch review (2026-08-27) — reachable from surface `tlprog!` syntax through public `compile`, not only a hand-built schedule**: `slotsBecomeScatter`'s diagonal detector counts only `.free`-marked axis UIDs (via `freeUID?`), so an LHS combining `.free a` and `.freeNorm a` on the SAME axis is not flagged scatter-shaped, yet `producerSlots` degrading `.freeNorm a → .free a` on the producer turns it into a genuine diagonal LHS no gate checked. Confirmed: `tlprog!{ Y[i, i.] := softmax(X[i]) }` is ACCEPTED by `TLProgram.compile`, while the control `tlprog!{ Y[i, i] := softmax(X[i]) }` is correctly REJECTED — same shape, one spelling silently mis-routed. Not yet closed; see `LeanNCD/DSL/Pipeline/RouteFragments.lean`'s header and the slice's SDD ledger for the reproduction and planned fix. |
