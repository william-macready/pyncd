...
- `papers/graded_prop.md` (categorical core),
- `docs/tensor_logic_dsl.md` (user-facing DSL),
- `leanncd/LeanNCD/DSL/Pipeline/*.lean` (actual passes).
...
From `papers/graded_prop.md`, we already have:
...
The heart of Naperian semantics is **representability** — a fundamental concept in category theory (see [MacLane 1998, Ch. III](#references)) where a functor is isomorphic to a hom-functor. In our setting, the isomorphism is:
...
- **$\text{El}(P) := D(I_D, P)$** denotes the **categorical ideal** of global elements or **points** of a shape object `P` in the index category `D`. Here `D(I_D, P)` is the **hom-set** — the set of all morphisms `I_D → P` in `D`, where `I_D` is the monoidal unit of `D` (written `I_D` throughout [graded_prop.md](#references); it is the unit for the `⊛` action, satisfying `X ⊛ I_D ≅ X`). A morphism `I_D → P` picks out a single "coordinate" or "element" of `P`, hence the name. In the concrete `StObj` setting, the full hom-set `D(I_D, P)` consists of all affine-stride morphisms `StMat [] P`; the `NaperianAxis StObj` instance uses a finite chosen representative set via `AxisPointData`, with `point_hom` providing the embedding. The categorical theory is stated for the full hom-set; the Lean implementation works with a finite representative. This is related to, but not identical with, the Elem notion from [graded_prop.md (Def 2.1)](#references): **(Elem-C)** is a separate statement that points jointly separate `C`-morphisms.
...
This representability is not coincidental. Following [Gibbons 2016](#references), for a **point functor** 
...
**Connection to elementality:** When the representability assumption holds, the evaluation transformations `ev_p := act(−, p)` (for $p \in \text{El}(P)$) form a jointly separating family for lifted `P`-indexed values. This Naperian joint monicity is not the same as the **(Elem-C)** axiom of [graded_prop.md](#references), which separates arbitrary `C`-morphisms by `C`-points. Naperian typing makes the lifted dense family explicit in the type system by:
...
The `TemporalGraded` mixin adds **causality**: output at time `t` depends only on times `≤ t`. This is a special case of the `D`-graded structure when the index category `D` is enriched with **directed structure** (see [directed objects](#references) in category theory literature). Categorically:
...
2. A **scan** is not a pointwise operation along `L`; it is a **finite fold** / **catamorphism** over the directed structure. Formally (from [Mathlib.Data.Functor.Foldable](#references)), it is a morphism out of the initial algebra of the prefix-indexed functor, encoding stepwise iteration with feedback.
...
1. **Soundness is structured** (from [Mathlib.CategoryTheory.Monoidal.Functor](#references)): If mixins respect strong monoidality, its coherence laws, and the separate representability assumptions, then **coherence isomorphisms** ($\delta, \delta_0, \upsilon, \alpha$) from [MacLane](#references) have standard categorical proof targets. Constructor-level quotient and extensionality proofs are still required in the Lean implementation.

2. **Proof obligations shift** (from direct application to categorical library): Rather than proving individual lemmas about reindexing or pointwise lifting, Lean implementations **inherit theorems from the categorical library** (e.g., [Mathlib.CategoryTheory.Monoidal.Functor](#references) for monoidal functor properties). This significantly reduces the sorry-count.
...
5. **[graded_prop.md](#references)** — Graded Colored PROPs: A Categorical Synthesis of St, Br, Acsets, and Algebras.  
   Located at: `papers/graded_prop.md`

6. **[tensor_logic_dsl.md](#references)** — The Tensor Logic DSL.  
   Located at: `docs/tensor_logic_dsl.md`
