...
- [Section B — Slice 2: Scatter + affine LHS writes — **SUPERSEDED**](#section-b--slice-2-scatter--affine-lhs-writes--superseded-2026-09-09)
...
- **DONE (2026-09-09): Section B was promoted into `papers/scatter_affine_lhs_writes.md`** and is now a stub. It was authored from the
...
write-geometry contract row as well as in `papers/scatter_affine_lhs_writes.md` §3.4. **Owner: the
deferred scan-scatter slice**, which
...
**What Slice 1 deliberately did NOT close**, now carried into `papers/scatter_affine_lhs_writes.md` §3.4: the free branch's `bias == 0`
...
## Section B — Slice 2: Scatter + affine LHS writes — **SUPERSEDED 2026-09-09**

> **This section is stale and has been replaced by `papers/scatter_affine_lhs_writes.md`.** Read
> that document instead. Everything below the line that was still true has been carried across;
> nothing here is authoritative any more.

**Why it was superseded, not edited.** Its central framing — that Scatter is one slice, to be
planned from Slice 1's compile-error list — is wrong. Measurement (probe, `lake env lean`) shows a
strided write into scan state is rejected at DSL phase 7 by `checkScatterNoScan` in **both** the
base and step blocks, so it is unreachable from surface syntax either way, while top-level
`Out[2*i] := X[i]` compiles through the whole DSL and dies at one checked-plan barrier over a
missing IR node. Those are two nearly-disjoint slices. **Slice 1's tripwire and the nine-site
compile-error list serve the deferred one.**

**Specific claims in this section that were measured wrong:**

| Claim | Correction |
|---|---|
| "Expect three rounds" of compile errors | **Two.** `ScanTest.lean` imports `Scan.lean` only, never `Compile.lean`, so both unblock in the same build |
| "four docstrings and one `AGENTS.md` node" enumerate the constructor set | **six prose sites + one node** |
| `allRowKinds`' risk is clause 1 losing its redundancy | the real risk is **frozen-copy coupling**; a deliberate divergence builds green at 8660 under the narrow list |
| the decision gating the work is base-vs-step | that decision chose between two **currently-unreachable** options; the real gate is whether checked-backend parity is the goal |

**What was carried across, so nothing is lost:** the witness-function mechanism for
`classifyWriteRow` and the two measured unpinned value checks are now in
`papers/scatter_affine_lhs_writes.md` §3.4; the extent rule, `allRowKinds`, barrier analysis and
the clause-3 extraction are in its §3.2–§3.4; `B2-F3`/`B2-F6`/`B1-F9` and the value-grep obligation
are in its §2.4 and §0.4.
...
- **Pushing to `origin`.** (Stale as written: `main` was pushed on 2026-09-09 and `main == origin/main` at `7e41710`.) The standing preference remains: do not push without an explicit request.
